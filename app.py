...
def modify_table_entries(previous_n_clicks,
                         next_n_clicks,
                         graph_relayout_data,
    ...
    if cbcontext == "graph.relayoutData":
        debug_print("graph_relayoutData:", graph_relayout_data)
        ...
        if "shapes" in graph_relayout_data.keys():
            ...
            annotations_table_data = [
                shape_to_table_row(sh) for sh in graph_relayout_data["shapes"]
            ]
        elif re.match("shapes\[\d]+\].x0", list(graph_relayout_data.keys())[0]):
